...
<div class="container py-4">
    ...
    @php
        // Tentukan prefix route sesuai role (case-insensitive)
        $role = strtoupper(Auth::user()->role);
        $prefix = $role === 'ADMIN' ? 'admin' : 'dokter';
    @endphp
        ...
        <div class="row">
            @foreach($polis as $poli)
                <div class="col-md-4 mb-4">
                    <a href="{{ route($prefix.'.janji_temu_offline.index', ['poli' => $poli->id]) }}"
        ...
        <form method="GET" class="row g-2 mb-4">
            ...
            <div class="col-md-4">
                <input type="text" name="nama" class="form-control"
                       placeholder="Cari Nama Pasien" value="{{ $nama }}">
            </div>
            ...
            <div class="col-md-2">
                <a href="{{ route($prefix.'.janji_temu_offline.index') }}"
                   class="btn btn-secondary w-100">Pilih Poli Lain</a>
            </div>
        </form>
            ...
            <div class="col-md-6">
                ...
                @if($janjiTemuOnline->count())
                    @foreach($janjiTemuOnline as $tanggalJanji => $items)
                        <h5 class="mt-3">
                            {{ \Carbon\Carbon::parse($tanggalJanji)->translatedFormat('l, d F Y') }}
                        </h5>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->status == 'Menunggu')
                                                <form action="{{ route($prefix.'.janji_temu_offline.panggil', $item->id) }}"
                                                      method="POST" class="d-inline">
                                                    @csrf
                                                    <button class="btn btn-sm btn-primary">Panggil</button>
                                                </form>

                                                <form action="{{ route($prefix.'.janji_temu_offline.batal', $item->id) }}"
                                                      method="POST" class="d-inline"
                                            ...
                                            @elseif($item->status == 'Diproses')
                                                <form action="{{ route($prefix.'.janji_temu_offline.selesai', $item->id) }}"
                                                      method="POST" class="d-inline">
                                                    @csrf
